 ...
 namespace Chess.Core
{
	public class Evaluation
	{
		...
		static readonly int[] passedPawnBonuses = { 0, 120, 80, 50, 30, 15, 15, 0 };
		...
		public int EvaluatePawns(int colourIndex)
		{
			...
			for (int i = 0; i < pawns.Count; i++)
			{
				...
				if ((opponentPawns & passedMask) == 0)
				{
					...
					// if (numSquaresFromPromotion < 0 || numSquaresFromPromotion >= passedPawnBonuses.Length) {
					// 	
					// 	numSquaresFromPromotion = System.Math.Clamp(numSquaresFromPromotion, 0, passedPawnBonuses.Length - 1);
					// }
