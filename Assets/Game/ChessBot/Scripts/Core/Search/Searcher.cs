...
namespace Chess.Core
{
	...
	public class Searcher
	{
		...
		{
			Debug.Log("DoIterativeDeepeningSearch");
			for (int searchDepth = 1; searchDepth <= 256; searchDepth++)
			{
				...
				else
				{
					...
					{
						Debug.Log("Mate found in iteration: " + searchDepth);
					...
					{
						Debug.Log("Mate found within search depth");
		...
		{
			var a = Search(settings.FixedSearchDepth, 0, negativeInfinity, positiveInfinity);
			Debug.Log("DoFixedDepthSearch " + a);
			bestMove = bestMoveThisIteration;
			bestEval = bestEvalThisIteration;
		}
		...
		public System.Span<Chess.Core.Move> GetMoves()
		{
